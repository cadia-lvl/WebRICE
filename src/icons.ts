...
abstract class icon{
    ...
    color = "white";
    size = "36px";
    constructor(id: string, color?: string, size?:string){
        this.ID = id;
    ...
    abstract createHtml(): SVGSVGElement;
    ...
    changeSize(size:string){
        const regex = new RegExp('^[0-9]+(cm)|(m{2})|(in)|(px)|(pt)|(pc)|(em)|(ex)|(ch)|(rem)|(vw)|(vh)|(vmin)|(vmax)|%$');
        if(regex.test(size)){
            const icon = document.getElementById(this.ID);
            if(icon){
